...
- [P1 — The APS MVP is unfinished and has not run its complete
  loop](problems/p1-finish-mvp-and-run-loop.md) — eleven concept reviews done
  (seven pruned); operations processes consolidated (4 groomings → 1,
  3 intakes → 1); learning loop wired in; current state now a first-class
  framework concept. See its `Current state` section.
...
- Current state promoted to a first-class framework concept: vocabulary
  entry, README references, changelog (`dfc62a4`).
- Two stream-declaration feedback records captured (`da58d36`, `80a9ed5`),
  not yet groomed.
